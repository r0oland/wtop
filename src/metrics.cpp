...
CpuSample MetricsCollector::sampleCpu() {
    ...
    if (prevIdle_ == 0) {
        prevIdle_ = i; prevKernel_ = k; prevUser_ = u; return {}; }
    ...
    prevIdle_ = i; prevKernel_ = k; prevUser_ = u;
    CpuSample cs;
    if (total) {
...
MemorySample MetricsCollector::sampleMemory() {
    MEMORYSTATUSEX ms{ sizeof(ms) }; MemorySample m;
...
std::optional<NetSample> MetricsCollector::sampleNet() {
    ...
    if (selectedNetInterface_ == -1) {
        ...
        for (DWORD i = 0; i < table->dwNumEntries; ++i) {
            ...
            if (!selected || row.dwSpeed > selected->dwSpeed) selected = &row;
        }
    } else {
...
std::optional<DiskSample> MetricsCollector::sampleDisk() {
    if (!diskInitialized_) return std::nullopt;
    auto nowTick = GetTickCount64();
    if (PdhCollectQueryData(reinterpret_cast<PDH_HQUERY>(pdhQuery_)) != ERROR_SUCCESS) {
        return std::nullopt;
    }

    PDH_FMT_COUNTERVALUE vRead{}; DWORD typeRead = 0;
    PDH_FMT_COUNTERVALUE vWrite{}; DWORD typeWrite = 0;
    if (PdhGetFormattedCounterValue(reinterpret_cast<PDH_HCOUNTER>(pdhReadCounter_), PDH_FMT_LARGE, &typeRead, &vRead) != ERROR_SUCCESS) return std::nullopt;
    if (PdhGetFormattedCounterValue(reinterpret_cast<PDH_HCOUNTER>(pdhWriteCounter_), PDH_FMT_LARGE, &typeWrite, &vWrite) != ERROR_SUCCESS) return std::nullopt;
